...
router.post('/login', CatchAsync(async (req,res,next) =>{
   ...
   const isMatch = await Bcrypt.compare(String(user.password), String(foundUser.password));
  ...
  const token = Jwt.sign(foundUser.toJSON(),"ChuckNorris",{expiresIn:"1h"});
...
router.post('/getJoke', userAuth, CatchAsync(async (req:UserRequest,res,next) => {
